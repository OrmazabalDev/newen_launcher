...
async fn load_instances(app: &AppHandle) -> Result<Vec<Instance>, String> {
    ...
    let parsed: Vec<Instance> =
        serde_json::from_str(trimmed).map_err(|e| format!("instances.json invalido: {}", e))?;
    Ok(parsed)
}

pub async fn get_instance_impl(app: &AppHandle, instance_id: &str) -> Result<Instance, String> {
...
async fn save_instances(app: &AppHandle, instances: &[Instance]) -> Result<(), String> {
    let path = instances_file(app);
    if let Some(parent) = path.parent() {
...
fn normalize_loader(loader: &str) -> Result<String, String> {
    let lowered = loader.trim().to_lowercase();
    match lowered.as_str() {
        "vanilla" | "snapshot" | "forge" | "fabric" | "neoforge" => Ok(lowered),
        _ => Err("Loader inválido (usa vanilla, snapshot, forge, neoforge o fabric)".to_string()),
    }
}

async fn count_mods(dir: &PathBuf) -> u32 {
...
pub async fn refresh_instance_mods_cache(
    app: &AppHandle,
    instance_id: &str,
) -> Result<u32, String> {
    let count = count_mods(&instance_dir(app, instance_id)).await;
...
pub async fn list_instances_impl(app: &AppHandle) -> Result<Vec<InstanceSummary>, String> {
    let instances = load_instances(app).await?;
    let mut out = Vec::new();
...
pub async fn create_instance_impl(
    ...
    if loader == "forge" && !req.version.contains("forge") {
        return Err("La versión Forge debe contener 'forge' en el ID".to_string());
    }
    if loader == "fabric" && !req.version.contains("fabric") {
        return Err("La versión Fabric debe contener 'fabric' en el ID".to_string());
    }
    if loader == "neoforge" && !req.version.contains("neoforge") {
        return Err("La versión NeoForge debe contener 'neoforge' en el ID".to_string());
    }
    if loader == "snapshot"
    ...
    {
        return Err("Las snapshots no pueden ser Forge/NeoForge/Fabric".to_string());
    }

    let versions_dir = get_launcher_dir(app).join("versions");
    let version_path = versions_dir.join(&req.version);
    if !version_path.exists() {
        return Err("La versión no está instalada".to_string());
    }
...
pub async fn update_instance_impl(
    ...
    for inst in &mut instances {
        if inst.id == instance_id {
            ...
            if let Some(thumbnail) = req.thumbnail {
                inst.thumbnail = if thumbnail.trim().is_empty() {
                    None
                } else {
                    Some(thumbnail)
                };
    ...
    let instance = found.ok_or("Instancia no encontrada".to_string())?;
    save_instances(app, &instances).await?;
    Ok(build_summary(app, &instance).await)
}

pub async fn delete_instance_impl(app: &AppHandle, instance_id: String) -> Result<(), String> {
    let _ =
...
pub async fn touch_instance_impl(app: &AppHandle, instance_id: &str) -> Result<(), String> {
    let mut instances = load_instances(app).await?;
...
pub fn open_instance_folder_impl(app: &AppHandle, instance_id: String) -> Result<(), String> {
