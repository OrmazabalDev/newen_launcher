...
pub(crate) fn resolve_game_dir(
    ...
    if version_id.contains("neoforge") {
        ...
        let path = base.join("profiles").join("neoforge").join(version_id).join(profile);
        return Ok(path);
    }
    if version_id.contains("-forge-") {
        let profile = forge_profile.unwrap_or("default");
        let path = base.join("profiles").join("forge").join(version_id).join(profile);
        return Ok(path);
    }
    Ok(base)
}

pub(crate) fn open_launch_log(base_dir: &Path) -> AppResult<(PathBuf, Stdio, Stdio)> {
    let logs_dir = base_dir.join("logs");
    fs::create_dir_all(&logs_dir).map_err(|e| crate::error::AppError::Message(e.to_string()))?;
    let log_path = logs_dir.join("launcher-latest.log");
    let file =
        fs::File::create(&log_path).map_err(|e| crate::error::AppError::Message(e.to_string()))?;
    let file_err = file.try_clone().map_err(|e| crate::error::AppError::Message(e.to_string()))?;
...
pub(crate) async fn ensure_disk_space(
    ...
    let target =
        if preferred_dir.exists() { preferred_dir.to_path_buf() } else { get_launcher_dir(app) };

    let target_clone = target.clone();
    let free =
        tokio::task::spawn_blocking(move || -> AppResult<u64> { free_space_bytes(&target_clone) })
            .await
...
fn free_space_bytes(path: &Path) -> AppResult<u64> {
    #[cfg(target_os = "windows")]
    {
        ...
        if ok == 0 {
            return Err("No se pudo obtener espacio libre".to_string().into());
        }
        Ok(free_bytes)
    }
    ...
    {
        let _ = path;
        Err("No se pudo obtener espacio libre".to_string().into())
    }
}
