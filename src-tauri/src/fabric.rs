...
fn build_fabric_url(base: &str, segments: &[&str]) -> Result<Url, String> {
    let mut url = Url::parse(base).map_err(|e| e.to_string())?;
    {
        let mut segs = url
            .path_segments_mut()
            .map_err(|_| "URL base invalida".to_string())?;
        for s in segments {
            segs.push(s);
        }
    }
...
async fn fetch_text_checked(client: &reqwest::Client, url: &str) -> Result<String, String> {
    let resp = client.get(url).send().await.map_err(|e| e.to_string())?;
    let status = resp.status();
    let text = resp.text().await.map_err(|e| e.to_string())?;
    if !status.is_success() {
        let preview: String = text.chars().take(200).collect();
        return Err(format!("Respuesta HTTP {}: {}", status, preview));
    }
    Ok(text)
}
...
pub async fn install_fabric_impl(
    ...
    let _ = app.emit(
        "download-progress",
        ProgressPayload {
            task: "Preparando Fabric...".to_string(),
            percent: 0.0,
        },
    );

    let mc_version_hint = normalize_version_hint(&mc_version);
    let mc_version = if !mc_version_hint.is_empty() {
        mc_version_hint
    } else {
        mc_version
    };
    ...
    let _ = app.emit(
        "download-progress",
        ProgressPayload {
            task: "Resolviendo loader Fabric...".to_string(),
            percent: 10.0,
        },
    );

    let client = create_client();
    let loader_url = build_fabric_url(
    ...
    let loader_text = loader_resp.text().await.map_err(|e| e.to_string())?;
    if status == reqwest::StatusCode::NOT_FOUND {
        return Err(format!(
            "No hay loader Fabric para Minecraft {}",
            mc_version
        ));
    }
    if !status.is_success() {
    ...
    let loader_version = if let Some(requested) = loader_override {
        ...
        if !hint.is_empty() {
            if let Some(found) = loader_entries
                .iter()
                .find(|e| matches_loader_hint(&e.loader.version, &hint))
            {
                found.loader.version.clone()
            } else {
                pick_latest_loader(&loader_entries)
                    .ok_or("No hay loader Fabric disponible".to_string())?
            }
        } else {
            pick_latest_loader(&loader_entries)
                .ok_or("No hay loader Fabric disponible".to_string())?
        ...
        pick_latest_loader(&loader_entries).ok_or("No hay loader Fabric disponible".to_string())?
    };

    let _ = app.emit(
        "download-progress",
        ProgressPayload {
            task: "Descargando perfil Fabric...".to_string(),
            percent: 40.0,
        },
    );
    ...
    let version_id = profile_json
        .get("id")
        .and_then(|v| v.as_str())
        .ok_or("Perfil Fabric invalido: falta id".to_string())?
        .to_string();
    ...
    fs::write(&json_path, profile_text.as_bytes()).map_err(|e| e.to_string())?;

    let _ = app.emit(
        "download-progress",
        ProgressPayload {
            task: "Fabric instalado".to_string(),
            percent: 100.0,
        },
    );
