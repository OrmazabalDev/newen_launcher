...
async fn read_active_skin(app: &AppHandle) -> AppResult<Option<SkinInfo>> {
    ...
    let meta = if let Ok(raw) = tokio_fs::read_to_string(&meta_path).await {
        serde_json::from_str::<SkinMeta>(&raw)
            .unwrap_or(SkinMeta { name: "Skin".to_string(), model: "steve".to_string() })
    } else {
        SkinMeta { name: "Skin".to_string(), model: "steve".to_string() }
    };

    let bytes =
        tokio_fs::read(&path).await.map_err(|e| crate::error::AppError::Message(e.to_string()))?;
    let data_url = format!("data:image/png;base64,{}", general_purpose::STANDARD.encode(bytes));
    Ok(Some(SkinInfo { name: meta.name, model: meta.model, data_url }))
}

pub async fn get_active_skin(app: AppHandle) -> AppResult<Option<SkinInfo>> {
...
pub async fn set_active_skin_base64(
    ...
    if !is_png_header(&bytes) {
        return Err("La skin debe ser un PNG válido".to_string().into());
    }
    ...
    tokio_fs::write(active_skin_path(&app), &bytes)
        ...
        .map_err(|e| crate::error::AppError::Message(e.to_string()))?;
    let meta = SkinMeta {
        name: if name.trim().is_empty() { "Skin".to_string() } else { name.trim().to_string() },
        model: if model.trim().is_empty() { "steve".to_string() } else { model.trim().to_string() },
    };
    ...
    let data_url = format!("data:image/png;base64,{}", general_purpose::STANDARD.encode(bytes));
    Ok(SkinInfo { name: meta.name, model: meta.model, data_url })
}
...
pub async fn set_active_skin_url(
    ...
    let resp = client
        ...
        .map_err(|e| crate::error::AppError::Message(e.to_string()))?;
    let bytes =
        resp.bytes().await.map_err(|e| crate::error::AppError::Message(e.to_string()))?.to_vec();
    if !is_png_header(&bytes) {
        return Err("La URL no apunta a un PNG válido".to_string().into());
    }
...
pub async fn clear_active_skin(app: AppHandle) -> AppResult<()> {
    let _ = tokio_fs::remove_file(active_skin_path(&app)).await;
    let _ = tokio_fs::remove_file(active_meta_path(&app)).await;
    Ok(())
}

pub async fn get_active_cape(app: AppHandle) -> AppResult<Option<String>> {
...
pub async fn set_active_cape_base64(app: AppHandle, data: String) -> AppResult<String> {
    let cleaned = clean_base64(&data);
    let bytes = general_purpose::STANDARD
        .decode(cleaned)
        .map_err(|_| "Datos de cape inválidos".to_string())?;
    if !is_png_header(&bytes) {
        return Err("La cape debe ser un PNG válido".to_string().into());
    }
    let dir = skins_dir(&app);
    tokio_fs::create_dir_all(&dir)
        .await
...
pub async fn set_active_cape_url(app: AppHandle, url: String) -> AppResult<String> {
    let client = crate::utils::create_client();
    let resp = client
        .get(&url)
        .send()
...
pub async fn clear_active_cape(app: AppHandle) -> AppResult<()> {
